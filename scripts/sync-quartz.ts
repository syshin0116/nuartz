...
const CORE_PACKAGE_DIR = './packages/core/src'

const isDryRun = process.argv.includes('--dry-run')

// Files/folders to sync from Quartz
const SYNC_TARGETS = [
...
async function main() {
  try {
    ...
    console.log(`📦 Quartz version: ${version}\n`)
    ...
    console.log('\nNext steps:')
    console.log('  1. Review changes: git diff packages/core/src')
...
async function cloneOrUpdateQuartz() {
  if (existsSync(UPSTREAM_DIR)) {
    console.log('📥 Updating Quartz from upstream...')
    if (!isDryRun) {
      await $`cd ${UPSTREAM_DIR} && git pull origin v4`
    }
    console.log('✓ Updated\n')
  } else {
    console.log('📥 Cloning Quartz repository...')
    if (!isDryRun) {
...
async function syncFiles() {
  ...
  for (const target of SYNC_TARGETS) {
    ...
    const destPath = join(CORE_PACKAGE_DIR, target.to)

    if (!existsSync(sourcePath)) {
      console.log(`⚠️  Skipping ${target.from} (not found in upstream)`)
      continue
    }

    console.log(`  ${target.from} → packages/core/src/${target.to}`)
...
async function updateVersionFile(version: string) {
  const versionFilePath = join(CORE_PACKAGE_DIR, 'version.ts')
...
`
  ...
  console.log(`\n✓ Updated version.ts (Quartz ${version})`)
}
