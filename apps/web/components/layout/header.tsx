        ...
        <div className="mx-auto flex h-14 w-full max-w-[1440px] items-center gap-2 px-4 sm:px-6">
          ...
          {search && (
            <Button variant="outline" size="sm" className="hidden gap-2 text-muted-foreground sm:flex" onClick={() => window.dispatchEvent(new Event("nuartz:search"))}>
              ...
              <kbd className="ml-4 rounded border px-1 text-[10px]">⌘K</kbd>
            </Button>
          )}
          {search && (
            <Button variant="ghost" size="icon" className="sm:hidden" aria-label="Search notes" onClick={() => window.dispatchEvent(new Event("nuartz:search"))}>
              <Search className="size-4" />
            </Button>
          )}
          ...
          <MobileNav tree={tree} title={title} links={links} darkMode={darkMode} />
        </div>
      </header>
    </TooltipProvider>
  )
